...
European robin _(Erithacus rubecula)_; one of the species in the Montrave study of \citet{Buckland_2006}.
```

This practical is based on the Montrave songbird case study in \citet{buckland_distance_2015}, with computer code under [Montrave songbird case study](https://synergy.st-andrews.ac.uk/ds-manda/#montrave-songbird-case-study). Both point and line transect surveys were conducted and here we use the data from the __line transect__ data, although the issues (and solutions) will be similar. 
...
## Advanced:  modularising R code to work with multiple species
...
The first function `augment.empty.transects()` performs the data manipulation described in Section \@ref{Manipulating-the-robin-data}, with two arguments: the data frame containing the full survey data and the species code on which to subset the data.

```{r emptytrans, echo=TRUE, eval=TRUE}
augment.empty.transects <- function(surveydata, species) {
  #  Purpose: find transects on which species not detected
  #           adjust data file accordingly such that effort is correct
  #  Input: raw data file, species on which to subset data
  #  Output: data frame ready to have detection function fitted (with correct effort)
  #  Rexstad August 2018

  num.transects <- length(unique(surveydata$Sample.Label))
  holes <- which(as.matrix(table(surveydata$species, surveydata$Sample.Label))==0, 
                 arr.ind = TRUE)  # row & col of transects without spec detect
  if (length(holes[rownames(holes)==species])==0) {    # species detected on all transects
    species.detects <- surveydata[surveydata$species==species, ]  
  } else {
    alltranlen <- vector(mode="numeric", length=num.transects)
    for (i in 1:num.transects) alltranlen[i] <- surveydata$Effort[surveydata$Sample.Label==i][1]
    ...
    for (i in 1:length(holes[rownames(holes)==species,2])) {
      ...
      empty.record <- cbind(surveydata[1,1:3], empty.label, empty.length)
      empty.transects <- rbind(empty.transects, empty.record)
    }
    empty.transects[ ,c("a", "b", "c")] <- NA
    names(empty.transects) <- names(surveydata)
    
    species.detects <- surveydata[surveydata$species==species, ]
    species.detects <- rbind(species.detects, empty.transects)
    species.detects <- species.detects[order(species.detects$Sample.Label), ]
  }
  return(species.detects)
}
```
...
```{r fitfn, echo=TRUE, eval=TRUE}
fit.hn.uni.haz <- function(data, trunc, print=TRUE) {
  #  Purpose:  fit three key functions to transect data, perform model selection
  #            print model selection table
  #  Input: data to analyse, truncation distance, print model selection table
  #  Output: fitted model object (class `dsmodel`)
  #  Rexstad August 2018
  hn.herm <- ds(data, truncation=trunc, transect="line", 
                      key="hn", adjustment="herm", convert.units=.1)
  uni.cos <- ds(data, truncation=trunc, transect="line",
                      key="unif", adjustment="cos", convert.units=.1)
  haz.simp <- ds(data, truncation=trunc, transect="line",
...
```{r usefunct, echo=TRUE, eval=answer, fig.width=3.5, fig.height=3.5, fig.margin=TRUE, results='hold', message=FALSE}
for(species in c("r", "c", "w")) {
  best.model <- fit.hn.uni.haz(
                               augment.empty.transects(birds, species),
                               100,
                               print=FALSE)
  plot(best.model, 
