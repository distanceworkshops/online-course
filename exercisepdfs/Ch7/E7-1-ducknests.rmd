...
```{marginfigure, echo=!answer}
Is there much difference in the probability of detecting a nest given it is within the maximum detection distance between the three models fitted to the duck nest data?
```

```{marginfigure, echo=answer}
No, there is barely any difference between the estimated probability of detection.  For the first two models, P_a_= `r round(nest.model1$ddf$fitted[1],3)` and for the hazard rate model, P_a_ = `r round(nest.model3$ddf$fitted[1],3)`.
```
