...
Bigeye binoculars used in a cetacean survey.
```
This tutorial is the first in a series that were created as an introduction for those wishing to undertake distance sampling and related methods in R \citep{r_language}. The basics of R and RStudio are explored in this tutorial. Data are generated resembling a set of perpendicular distances collected from a track line and basic R commands are used to summarise and analysis it.
...
```{r, echo=TRUE, eval=answer, fig.margin=TRUE, fig.height=3.5, fig.width=3.5}
setbreaks <- seq(from=0, to=8, by=0.5)
...
```{r, echo=FALSE, eval=answer, fig.margin=TRUE, fig.width=3.5, fig.width=3.5}
...
temp <- hist(truncdists, breaks=setbrks, plot=FALSE)
maxy <- max(temp$counts)
maxx <- 6
...
cat('The prob. of detection with truncation is ',round(truncp,4))
```
...
We can use the `Distance` package \citep{distance_pkg} to fit a detection function and estimate the probability of detecting objects given they are within the truncation distance of the transect.

There are hundreds of R packages and so for efficiency the more specialised packages are not installed as standard when R is downloaded onto your computer. To check whether the `Distance` package is installed, click on the 'Packages' tab (lower right pane) and see if it is listed. If so, then either click in the tick box or load it using the command
...
```{r, echo=TRUE, eval=answer, fig.margin=TRUE, fig.height=3.5, fig.width=3.5, fig.cap="Half normal detection function fitted to all simulated data."}
plot(dsmodel)
```

To truncate the distances, the `truncation` argument in `ds` can be used:
...
```{r, echo=FALSE, eval=answer, message=FALSE, fig.margin=TRUE, fig.height=3.5, fig.width=3.5, fig.cap="Half normal detection function fitted to simulated data truncated at distance=6."}
summary(dstrunc)
plot(dstrunc)
```
